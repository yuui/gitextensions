...
using GitExtensionsSccProvider;
using Henk.GitExtensionsSccProvider.Git;
...
namespace Henk.GitExtensionsSccProvider
{
        ...
        IVsSccGlyphs*/
    {
        ...
        public GitExtensionsSccProviderPackage Package { get; private set; }

        public GitSourceControlProvider(GitExtensionsSccProviderPackage package)
        {
            Package = package;
        }

        private void indexWatcher_Changed(object sender, EventArgs e)
        {
            RefreshAllGlyphs();
        }
        ...
        {
            if (string.IsNullOrEmpty(rgpszFullPaths[0]))
                return VSConstants.S_OK;

            Initialize();

            GitItemStatus fileStatus = GitFileStatus.GetFileStatus(rgpszFullPaths[0]);
        ...
        public int GetCustomGlyphList(uint BaseIndex, out uint pdwImageListHandle)
        {
            try
            {
                if (glyphList != null && BaseIndex != baseIndex)
                {
                    glyphList.Dispose();
                    glyphList = null;
                }
                ...
                {
                    baseIndex = BaseIndex;
                    glyphList = StatusIcons.CreateStatusImageList();
                    for (int i = (int)BaseIndex - 1; i >= 0; i--)
                    {
                        glyphList.Images.RemoveAt(i);
                    }
                }
                pdwImageListHandle = unchecked((uint)glyphList.Handle);
        ...
        {
            string solution = GetSolutionFileName();

            if (!solution.Equals(GitCommands.Settings.WorkingDir, StringComparison.CurrentCultureIgnoreCase))
            {
                LoadSettings.Load();
                GitCommands.Settings.WorkingDir = GetSolutionFileName();
